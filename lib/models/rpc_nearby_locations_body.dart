...
class RpcNearbyLocationsBody {
  ...
  static List<RpcNearbyLocationsBody> listFromJson(List<dynamic> json) {
    return json == null ? new List<RpcNearbyLocationsBody>() : json.map((value) => new RpcNearbyLocationsBody.fromJson(value)).toList();
  }

  static Map<String, RpcNearbyLocationsBody> mapFromJson(Map<String, Map<String, dynamic>> json) {
    var map = new Map<String, RpcNearbyLocationsBody>();
    if (json != null && json.length > 0) {
      json.forEach((String key, Map<String, dynamic> value) => map[key] = new RpcNearbyLocationsBody.fromJson(value));
    }
    return map;
  }
}
